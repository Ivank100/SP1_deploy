...
/**
 * This component renders the home page page UI.
...
type HomePageViewProps = {
  page: ReturnType<typeof useHomePage>;
};

export default function HomePageView({ page }: HomePageViewProps) {
  const {
    courseFormError,
    courseTab,
    creatingCourse,
            ...
            {viewMode === 'mindmap' ? (
              <div className="relative min-h-[400px] py-4">
                ...
                <div className="relative pl-2">
                  <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300" aria-hidden />
                  {sortedSemesterKeys.map((term, termIdx) => {
                    ...
                    const ConnectorSvg = ({ count, side }: { count: number; side: 'left' | 'right' }) => {
                      const width = 28;
                      const branchLen = 10;
                      const cy = 50;
                      ...
                      const paths: string[] = [];
                      if (count === 1) {
                        paths.push(`M ${side === 'left' ? width : 0},${cy} H ${side === 'left' ? 0 : width}`);
                      } else {
                        const stemX = side === 'left' ? branchLen : width - branchLen;
                        paths.push(`M ${side === 'left' ? width : 0},${cy} H ${stemX}`);
                        const yVals = Array.from({ length: count }, (_, index) => ((index + 0.5) / count) * 100);
                        paths.push(`M ${stemX},${Math.min(...yVals)} V ${Math.max(...yVals)}`);
                        yVals.forEach((y) => paths.push(`M ${stemX},${y} H ${side === 'left' ? 0 : width}`));
                      }
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 flex items-center justify-center gap-2 min-h-[11rem]">
                          ...
                          {isExpanded && <ConnectorSvg count={leftCourses.length} side="left" />}
                        </div>
                        <div className="relative shrink-0">
                          ...
                          <button
                            ...
                            className="absolute -bottom-3 left-1/2 -translate-x-1/2 rounded-full border border-gray-200 bg-white px-3 py-1 text-xs text-gray-600 shadow-sm"
                          >
                            {hiddenTermKeys.includes(term) ? 'Unhide term' : 'Hide term'}
                          </button>
                        </div>
                        <div className="flex-1 flex items-center justify-center gap-2 min-h-[11rem]">
                          {isExpanded && <ConnectorSvg count={rightCourses.length} side="right" />}
                ...
                {sortedSemesterKeys.length === 0 && <p className="text-sm text-gray-500">No courses to show.</p>}
              </div>
            ) : (
              <div>
                <div className="flex items-center justify-between mb-8">
                ...
                <div className="space-y-8">
                  ...
                  {visibleCourses.length === 0 && <p className="text-sm text-gray-500">No courses to show.</p>}
                </div>
              </div>
            )}
          </>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={resetCreateCourseForm} className="text-gray-400 hover:text-gray-600">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) => setNewCourseTermYear(Number(event.target.value))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    placeholder="2025"
                    required
                  />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) => setNewCourseTermNumber(Number(event.target.value) as 1 | 2)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  >
