...
def get_chunks_for_page(
    lecture_id: int,
    page_number: int,
) -> List[Tuple[str, Optional[int], int, str, str, Optional[float], Optional[float], Optional[float]]]:
    init_schema()
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
            ...
            WHERE c.lecture_id = %s AND c.page_number = %s
            ORDER BY c.chunk_index
            """,
            (lecture_id, page_number),
        )
        return cur.fetchall()
...
def search_by_reference_patterns(
    patterns: List[str],
    top_k: int = 8,
    lecture_id: Optional[int] = None,
    course_id: Optional[int] = None,
    ...
    """
    Find chunks matching document-structure references such as "question 20"
    and include nearby chunks for more complete local context.
    """
    if not patterns:
        return []
    ...
    init_schema()
    with get_conn() as conn, conn.cursor() as cur:
        match_clauses = " OR ".join(["lower(c.text) ~ %s"] * len(patterns))
        params: List[Any] = list(patterns)

        filters: List[str] = []
        if lecture_id is not None:
            filters.append("c.lecture_id = %s")
            params.append(lecture_id)
        elif course_id is not None:
            filters.append("l.course_id = %s")
            params.append(course_id)
        ...
        query = f"""
            WITH matching AS (
                ...
                FROM chunks c
                JOIN lectures l ON c.lecture_id = l.id
                WHERE ({match_clauses}){where_sql}
                GROUP BY c.lecture_id, c.chunk_index, c.page_number
                ORDER BY match_score DESC, c.page_number NULLS LAST, c.chunk_index
                LIMIT %s
            )
            SELECT DISTINCT ON (n.lecture_id, n.chunk_index)
                n.text,
                n.page_number,
                n.lecture_id,
                l.original_name,
            ...
            FROM matching m
            JOIN chunks n
              ON n.lecture_id = m.lecture_id
             AND n.chunk_index BETWEEN m.chunk_index - %s AND m.chunk_index + %s
            JOIN lectures l ON n.lecture_id = l.id
            ORDER BY
                n.lecture_id,
                n.chunk_index,
                ABS(n.chunk_index - m.chunk_index),
                m.match_score DESC,
                n.page_number NULLS LAST
            LIMIT %s
        """
